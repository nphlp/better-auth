---
icon: authenticationEmailPassword
...
On transactional adapters, native password reset and the server-only `setPassword` API serialize
credential creation through the same user-row write lock. A reset racing with initial password
creation therefore cannot insert a second credential. Account hooks remain active and a veto
fails the mutation. Nontransactional adapters keep their existing behavior without this
cross-process guarantee. Custom initial-password endpoints can call
`internalAdapter.setCredentialPassword(userId, hash, { overwrite: false, requireTransaction: true })`
after enforcing their session, verification and password policy.
...
The no-account callback also receives the optional `redirectTo` submitted to the endpoint, after
origin validation. Use it to build a registration invitation on an allowed frontend and preserve
a safe return target. It is a password-reset callback URL, so consumers should extract their own
return parameter rather than use it directly as the registration destination. The request body
may already be consumed; read `redirectTo` from the callback data.
