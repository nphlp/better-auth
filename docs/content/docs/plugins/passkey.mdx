...
### Requiring local user verification

Set `requireUserVerification: true` to request `userVerification: "required"` for registration and
authentication and enforce it during both server verifications. This overrides a weaker registration
`authenticatorSelection.userVerification`. A client changing the generated options cannot disable
the server check. Missing user verification rejects the ceremony without creating a passkey or session.
The default is `false`, preserving the existing preferred/optional behavior.
