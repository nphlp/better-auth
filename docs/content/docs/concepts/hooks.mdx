...
<Callout>
  Lifecycle callbacks use `runInBackgroundOrAwait` internally — they complete before the response when no background task handler is configured, or run asynchronously when one is. See [backgroundTasks](/docs/reference/options#backgroundtasks) for more details.
</Callout>
...
Triggered when a user signs in and a new session is created, regardless of the authentication method (email/password, magic link, OAuth, passkey).
...
Triggered when a user signs out and their session is deleted.
...
Triggered when a user changes their password from their profile.
...
Triggered when a verification email is sent. Runs alongside the `sendVerificationEmail` callback. Use for logging or analytics.
