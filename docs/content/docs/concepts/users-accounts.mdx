...
* **`user.pendingEmail`** holds the address awaiting confirmation, so your UI can show it.
  The column is only added to the user table under this strategy.
...
`pendingEmail` decides which **address** is current, which is what makes older links inert:

* Requesting a **different** address, or cancelling, invalidates every link issued before —
  they are rejected on use and their rows expire.
* Requesting the **same** address again leaves the earlier link valid too. Both lead to the
  same result, so there is nothing to arbitrate between them.
* An **expired** link never applies the change and leaves `pendingEmail` untouched, so the
  user can ask for a fresh email without retyping the address.
